...
import team57.project.service.impl.ClinicAdminServiceImpl;
import team57.project.service.impl.ClinicServiceImpl;
import team57.project.service.impl.ClinicalCenterAdminServiceImpl;
...
public class ClinicalCenterAdminController {
    ...
    @Autowired
    private ClinicalCenterAdminServiceImpl clinicalCenterAdminService;
    ...
    public ResponseEntity<?> saveClinicAdmin(@RequestBody UserDTO userDTO, @PathVariable Long id_clinic) {
    ...
    @PreAuthorize("hasRole('CLINICAL_CENTER_ADMIN')")
    public ResponseEntity<?> saveClinicalCenterAdmin(@RequestBody UserDTO userDTO) {
    ...
    public ResponseEntity<?> saveClinic(@RequestBody ClinicDTO clinic) {
        ...
        if (existClinic == null)
        {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Clinic with that name already exists in the system.");
        }
    ...
    public ResponseEntity<?> acceptRequest(@PathVariable Long id, UriComponentsBuilder ucBuilder)
    {
        ...
        if(p!=null)
        {
            ...
            try {
                ...
                logger.info("Sending an email with activation link to the user failed: " + e.getMessage());
            }
