...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendMessageApproved(Absence absence) throws MailException, InterruptedException, MessagingException {
        ...
        String msg = "Hello, " + name + "!\n\nYour request for the absence has been approved.\n\nBest regards,\nClinic admin";
        mail.setText(msg);
        mail.setTo(email);
    ...
    public void notificationAppointmentReq(Patient patient, Doctor doctor, MedicalExam medicalExam, Set<ClinicAdmin> admins) throws MailException, InterruptedException, MessagingException{
        ...
        for(ClinicAdmin admin : admins){
            mail.setText("Hello, " + admin.getName() + "!\n\n" + "Patient : " + patient.getName() + " " + patient.getSurname() +
                    " with serial number : " + patient.getSerialNumber() + " has send the request for appointment!\n\n" +
    ...
    public void sendNotificationForReservation(MedicalExamRequest merNew,MedicalExamRequest merOld, RoomME roomME,String emailP) throws MessagingException {
        ...
        if(!merNew.getDate().equals(merOld.getDate()) || !merOld.getStartTime().equals(roomME.getStartTime())){
            msg += "Date of you reservation for the medical exam - " + merNew.getExamTypeName() + " was changed on " + merNew.getDate().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG)) + " at " + roomME.getStartTime().format(DateTimeFormatter.ofPattern("hh:mm"))  +  ". Doctor: " + merNew.getDoctor().getFullName() + ".<br>";
            changed = true;
        }
        ...
        String message = "";
        if(!changed){
            message += "Hello, " + merNew.getFullNamePatient() + "<br>" + "You have successfully reserved the medical exam - " + merNew.getExamTypeName() + " on " + dateOld + " at " + timeOld + " in room " + roomME.getName() + ". Doctor: " + merNew.getDoctor().getFullName() + ".<br><br>";
        }else{
    ...
    @Async
    public void sendRejectSurgery(Surgery s, Patient p, SurgeryType st) throws MailException, InterruptedException, MessagingException{
        String date = s.getDate().format(DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG));
        SimpleMailMessage mail = new SimpleMailMessage();
        mail.setText("Hello, " + p.getName() + "!\n\n" + "Your request for the surgery- " + st.getName() + " on " + date +  " has been rejected because currently it's not possible to find a free doctor and a free room in this and the next week."  + "\n\nBest regards,\nClinic admin");
        mail.setTo(p.getEmail());
        mail.setSubject("Clinical Center System - Surgery rejection");
        mail.setFrom(env.getProperty("spring.mail.username"));
        javaMailSender.send(mail);
